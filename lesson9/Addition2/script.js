...
function newUserBlock(item,box,container){
    box.classList.add('box');
    for (const itemKey in item) {
        if(typeof item[itemKey]!=='object'){
            let p=document.createElement('p');
            p.innerText=`${itemKey.toUpperCase()}---${item[itemKey]}`;
            box.appendChild(p);
        }else {
...
for (const user of usersList) {
    ...
    newUserBlock(user,userBlock,mainContainer);
}
...
function searchClass(element) {
    if(element.tagName==='P'){
        paragraphs.push(element);//regex.test(field.value
...
let divmagic=document.getElementById('specialDiv');
window.addEventListener('keydown',(e)=>{
    let text=divmagic.innerText;
    if(e.key==='Tab')divmagic.innerHTML=`<${text}></${text}>`
})